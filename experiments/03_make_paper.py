...
def write_table(tid: str, df: pd.DataFrame, caption: str, prov: Provenance,
    ...
    # The note stays with the table; the provenance does not. A git sha and a timestamp
    # under every table is a reproducibility device, not a result, and repeating it
    # eleven times is noise a reader has to read past. It is stated once, in the
    # reproducibility appendix, from the same Provenance object.
...
def main() -> int:
    ...
    # One provenance statement for the whole artifact set, inputted by Appendix C.
    TABLES.joinpath("provenance.tex").write_text(
        "% Auto-generated by experiments/03_make_paper.py -- do not edit by hand.\n"
        "Every table and figure in this paper was produced by a single run of the\n"
        "release, recorded as: \\texttt{" + prov.footnote() + "}. The same identifier\n"
        "is repeated as a comment at the head of each generated file, so any table can\n"
        "be traced to the code, configuration, data and seeds that produced it.\n",
        encoding="utf-8")
    print("  wrote provenance.tex")
