...
\geometry{margin=1.7cm,columnsep=5.5mm}
...
\title{\bfseries Calibration Is the Binding Constraint on Position Sizing}
...
once a forecast becomes a position. \emph{Calibration} --- the agreement
between stated probability and realised frequency --- is, we argue and then
measure, the binding constraint on growth-optimal position sizing, and we
quantify what miscalibration costs. Analytically, the expected log-growth
penalty incurred by a miscalibrated forecast equals one half the squared
standardised error of the conditional mean, and underestimating the
conditional variance by a factor of two eliminates the entire growth advantage
of a genuinely profitable signal. Built around that result is
...
paper's thesis showing up as a measurement instead of an argument.
...
paper's central empirical finding, not a disappointing one: a decision
...
\item \textbf{A compact gated cross-modal architecture} with modality dropout,
  ...
  pathway is offered for replication, not demonstrated as a result.
...
\item \textbf{A leakage-controlled evaluation protocol}, pre-specified in
  ...
  Section~\ref{sec:validation} that its components are load-bearing and not
  merely ceremonial.
\item \textbf{A null result, reported as one.} On \kwTickerDays{} ticker-days
  ...
  report it as it stands, instead of searching for the configuration that
  would not have produced it. Along the way the protocol caught three defects in our own
  ...
  sized the level of the conditional mean instead of its cross-sectional
  deviation.
...
barrier-odds Kelly rule. The supplementary material lists the specification
conflicts that reconciliation resolved, since several changed the meaning of
reported quantities and not merely their values, along with the test that
...
The reference point is \citet{gu2020empirical}, which establishes both the
achievable signal level and the field's evaluation conventions;
\citet{kellyxiu2023} survey the area, \citet{chen2024deep} extend it with deep
conditional asset-pricing models, and \citet{jiang2023reimagining} show that
convolutional encoders over price charts recover recognisable technical
structure. \citet{fischer2018deep} give the recurrent benchmark closest to a
daily directional model. The consistent finding --- that flexible models do not
dominate regularised linear ones at this signal-to-noise ratio --- is the
empirical basis for the capacity argument of Section~\ref{sec:capacity}.

\paragraph{Text and returns.}
\citet{tetlock2007giving} established that negative word counts in a widely
read column predict returns and volume, and \citet{loughran2011liability}
showed that general-purpose lexicons misread financial language. Transformer
encoders adapted to financial corpora followed
\citep{araci2019finbert,yang2020finbert,huang2023finbert}; at least three
distinct models are called FinBERT, so we state our checkpoint explicitly in
Appendix~\ref{app:hyper}. \citet{ke2019predicting} extract return-predictive
content with a supervised topic model and \citet{lopezlira2023chatgpt} report
that instruction-tuned models score headlines competitively.
\citet{dong2024fnspid} and \citet{zhou2021trade} provide timestamped,
ticker-linked corpora that would make the text modality reproducible;
...
vision-and-language work \citep{lu2019vilbert,chen2020uniter}, and our gate
descends from gated multimodal units \citep{arevalo2017gated}. In finance,
\citet{hu2018listening} pair a news encoder with attention over price sequences
and \citet{sawhney2021quantitative} attend between transcripts and price
graphs. Our block is deliberately shallow --- one block, four heads --- and
pairs the gate with modality dropout, which turns the fusion ablation from a
...
piecewise constant in the parameters and has no useful gradient, so
training-time calibration needs a surrogate; we use the kernel statistic of
\citet{kumar2018trainable}. \citet{mukhoti2020calibrating} show focal loss
alters calibration on its own, which is why we keep it off by default. For
set-valued prediction we use split conformal
...
\citet{bailey2014deflated} and \citet{bailey2017probability} for the Deflated
Sharpe Ratio and the Probability of Backtest Overfitting;
...
Worked through, the rule says: a headline published at 09:00 on Tuesday, before
that day's open, informs the decision executed at Wednesday's 09:30 open and
earns the return from Wednesday's open onward. One published at 16:30 on
Tuesday, after the close, is \emph{not} usable at Wednesday's open --- only 17
hours have elapsed --- so it informs Thursday. A weekend headline informs
Monday. The second case is the one most often got wrong.
...
makes precise the sense in which it is a \emph{boundary} effect and not a
global one --- a distinction descriptions of the technique often blur.
...
state, not an edge case. Those rows receive a learned null embedding
...
would be a selection bias. News coverage per ticker-day is reported in the supplementary material.
...
instead of the sixty-day trend. The gate then decides, per dimension, how much
...
This buys three things. Collapse onto a single modality, the usual failure of
gated fusion when one input dominates, is prevented. A live news feed that
fails degrades gracefully instead of triggering an unhandled deployment
state. And it makes the single-modality ablation an \emph{evaluation-time}
intervention on one set of weights, not a comparison between separately
...
\subsection{Capacity}
\label{sec:capacity}
The supplementary material gives the realised parameter count by module. It is
emitted by the implementation and asserted against the specification by the test
suite, so it cannot drift from the model that produced our numbers. Roughly
...
The realised parameter count by module is given in the supplementary
material, emitted by the implementation and asserted against the
specification by the test suite.
...
the paper is left with a validated quantitative law, not just an intuition.
...
and not chosen after seeing the data.
...
protocol section, not buried in a footnote.
...
substitute: a retrospective scrape returns what is indexed today, not
...
The supplementary material reports the panel and states the corpus audit.
...
instead of deleting them, because a baseline that disappears between protocol
...
omitted, and that omission is ours, not the data's: both forecast
...
modulate. Section~\ref{sec:results} records that outcome instead of omitting
...
and never crosses a boundary, and invisible to inspection of the indicator
...
in advance, and it needs to be treated as a trigger, not a target to clear.
...
evaluation position --- instead of relying on an aggregate learning signal.
...
Fitted against that nearly flat likelihood, the six-parameter map latched onto
the validation fold's class balance instead, which then drifted: one fold had
a \Down{} share of $0.28$ in validation against $0.46$ in test. Left
unconstrained, the fit also returned scales such as
$(-0.019,-2.667,3.313)$, whose negative entries reverse the model's own class
ordering. Treat post-hoc calibration as model selection
...
are emitted by the released pipeline, and figures quoted in running text are
macros read from the same run, so prose and tables cannot disagree.
Appendix~\ref{app:hyper} records the commit, configuration, data and seeds
behind the whole artifact set.
...
The supplementary material breaks the panel down by year: universe size,
ticker-days, sessions and label class balance. Two features of it condition
every later number.
...
\Neutral{}, while posting the highest rank IC in the table --- a reader ranking
these six models by accuracy would order them almost backwards.

That is close to the thesis. Accuracy scores a decision the model is not asked
to make: a cross-sectional book never classifies, it ranks, and trades the top
against the bottom. Rank IC with a Newey--West $t$ is the column that speaks to
that, and it is why the majority-class row is mandatory --- without it, an
accuracy of \kwPrimaryAcc{} reads as weak evidence of something when it is
really clear evidence of nothing.
...
rank IC is undefined, not zero.}

\subsection{Calibration and conformal coverage}

The calibration results are sharper than the predictive ones, which is neither
surprising nor a consolation prize: a model can be perfectly calibrated while
being perfectly uninformative, by stating the base rate every time. That is
close to what happens here, and it is why the guarantee in
...
calibration and test blocks, and a decade of walk-forward does not supply it:
split coverage misses nominal by \kwCovMeanAbsSplitGap{} averaged over the
$\alpha$ grid and undershoots at \emph{every} level, while the online update of
\citet{gibbs2021adaptive} misses by \kwCovMeanAbsAdaptiveGap{} and lands on
\kwCovATenAdaptive{} against a nominal \num{0.90}. We expected to report this
as a caveat and report it as a result: here the adaptive variant is not a
refinement of the split one, it is the one that works.
...
it is a property of the model's uncertainty, not of the guarantee.}
...
not as zero: declining to bet is not a bet that returned nothing.}
...
friction, and it is why we report the whole curve instead of a net Sharpe
quoted at whichever cost flatters the result.

The Deflated Sharpe Ratio is computed against the declared trial budget, not
against one chosen after the fact: we evaluated a model zoo, two horizons beyond the primary one
and a rebalance sweep, and the deflation charges us for all of them. We claim
no risk premium --- the alpha $t$-statistic does not approach the threshold of
...
The supplementary material varies the three things most likely to be
responsible for a result that exists in one configuration only: subperiod,
horizon and seed. Rank IC is positive in every year of the sample and clears
$t=2$ in none of them; net Sharpe is positive in one year of seven. All three
sweeps are declared trials, charged to the Deflated Sharpe budget --- a
robustness table assembled after the fact and not counted against the trial
budget is a menu, not a control.


\begin{figure}[t]
\centering
\IfFileExists{figures/F5_main.pdf}
  {\includegraphics[width=0.49\linewidth]{figures/F5_main.pdf}\hfill}{}%
\IfFileExists{figures/F6_main.pdf}
  {\includegraphics[width=0.49\linewidth]{figures/F6_main.pdf}}
  {\fbox{\parbox{0.94\linewidth}{\centering\vspace{16mm}
...
\caption{Left: reliability of the proposed model's probabilities before and
after the guarded post-hoc map; the curve is short because a model near the
base rate emits confidences over a narrow range, and that narrowness is the
finding. Right: log-growth loss against calibration error, the bound of
...
coefficient is \kwPrimaryIC{}, with a Newey--West $t$ of \kwPrimaryICt{}, and the
gap between any two ablations of a model like that is simply the gap between
two noise terms.
...
We did not find a tradeable edge, and the scope of that statement is narrow: six
models, one universe of \kwNames{} liquid US names, a five-session horizon, one
decade. It is not a claim that short-horizon cross-sectional prediction is
impossible, and certainly not that news carries no information about returns ---
we could not test that proposition at all.

What the null does establish is more useful. A protocol built to resist its own
optimism returns nothing when there is nothing there, and every component of
this pipeline had the opportunity to manufacture a result instead. The
execution-lag defect of Section~\ref{sec:lagdefect} was worth an apparent rank
IC of \num{0.108} on data constructed to contain no signal, more than an order
of magnitude above anything we measure. Unguarded post-hoc calibration was
worth a tenfold apparent improvement while degrading held-out calibration.
Sizing on an uncentred conditional mean pinned every position to its cap. A
verified control caught each; any of the three, left alone, would have yielded
a number worth writing up.
...
places --- \kwCovAFive{} against a nominal \num{0.95} --- assuming nothing about
the model's accuracy, which is what a distribution-free guarantee is for. What
follows is uncomfortable and instructive: the model has no edge, so its
prediction sets are almost never singletons, so the gate declines every name
and holds \kwGateNamesHeld{} positions. That leaves the gated book returning
nothing, and nothing was the best outcome available, because the three ungated
rules all lost money. There is nothing clever in this: the gate is honest about the model's
uncertainty, and under a null that is enough to win.
...
event, and an evaluation protocol whose controls are verified, not merely
...
\item \textbf{Costs are modelled, not measured.} Equation~\eqref{eq:costs} is a
  model; we report a curve instead of a point because realised execution is
...
diagnostics instead of personalised recommendations, and any deployment would
require a documented governance framework, audit logging and a model-risk
process. We state this outright instead of relying on a disclaimer, because the
...
evaluation protocol whose controls we verified and did not simply assert.
...
Provenance is recorded once, here, instead of being repeated beneath every
table. A commit hash and a timestamp under each generated table is a
reproducibility device, not a result, and a reader would otherwise have to
read past it on every page. The identifier below covers the whole artifact
set, and it is also written as a comment at the head of every generated file,
so any single table can still be traced back on its own.
...
Three appendices are provided as a companion document: the reconciled
specification table listing every conflict resolved against the authors'
earlier design and the test that prevents each recurring; the full indicator
list with the conventions that cause hand-rolled implementations to drift; and
the generative-augmentation protocol. None is required to follow the results.
