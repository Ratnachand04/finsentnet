...
\subsection{Capacity}
\label{sec:capacity}
Table~\ref{tab:T2_main} gives the realised parameter count by module, emitted
by the implementation rather than estimated, and asserted against the
...
belong to the text pathway --- the projection, the attention pooling, the
source and null embeddings. In the forced price-only state those parameters are
carried but never receive a gradient from a real headline, so the model that
actually produced our results has \kwParamsPrice{} live parameters and the
effective samples-per-parameter ratio is correspondingly more comfortable than
the design intended. We report the full count because it is the specified
architecture, and we state the split here because a capacity argument that
quietly counts inert parameters is overstating the model in one direction and
...
The first is the class balance by year. The dead band is scaled by a causal
volatility estimate rather than fixed, which is what keeps the label from
degenerating into a volatility detector, but it does not make the three classes
equal: the \Up{} share exceeds the \Down{} share in most years, because the
sample covers a decade in which US equities rose. That asymmetry is why the
majority-class row in Table~\ref{tab:T3_main} is not decorative, and it is also
what broke our first attempt at position sizing --- a detail we record in
Section~\ref{sec:discussion} rather than quietly fix, because the failure is
instructive.
...
which is close to the $h+1$ the horizon implies. Every weighted loss and every
standard error in this paper uses the corrected figure. A reader who compares
our sample size to a paper that quotes raw rows should divide by that factor
first.
...
The break-even cost is the first. It answers the only question a practitioner
asks of a backtest --- how much friction the signal can absorb before it stops
paying --- and it is reported here rather than a single net Sharpe at a
flattering assumption. A negative break-even cost is not a small edge; it means
the book loses money gross, before a single basis point is charged, and no
execution improvement can rescue it.
