...
reported quantities rather than merely their values, along with the test that
...
accuracy of \kwPrimaryAcc{} reads as weak evidence of something rather than
clear evidence of nothing.
...
friction, and it is why we report the whole curve rather than a net Sharpe
quoted at whichever cost flatters the result.

The Deflated Sharpe Ratio is computed against the declared trial budget rather
than against one: we evaluated a model zoo, two horizons beyond the primary one
...
the model's accuracy, which is what a distribution-free guarantee is for. The
practical consequence is uncomfortable and instructive. The model has no edge,
so its prediction sets are almost never singletons, so the gate declines every
name and holds \kwGateNamesHeld{} positions. The gated book returns nothing, and
nothing was the best outcome available, because the three ungated rules all lost
money. There is nothing clever in this: the gate is honest about the model's
