...
\title{\bfseries Calibration-Aware Cross-Modal Learning for Equity
Direction:\\[2pt] Gated News--Price Fusion, Conformal Abstention,\\
and Growth-Optimal Position Sizing}
...
\num{0.003} to \num{0.056}.
\ifresultsready
Empirical results follow in Section~\ref{sec:results}.
\else
The full walk-forward study is specified in Section~\ref{sec:protocol}; its
results are not yet reported. Section~\ref{sec:validation} reports the protocol
validation that precedes it. Code, configurations, seeds and 207 tests are
released.
\fi
\end{abstract}
...
\item \textbf{A compact gated cross-modal architecture} with modality dropout,
  ...
  effectively independent training samples (Section~\ref{sec:model}).
...
\item \textbf{A leakage-controlled evaluation protocol}, pre-specified in
  ...
  Section~\ref{sec:validation} that two of its components are load-bearing
...
\item \textbf{A released, tested implementation.} The data, evaluation,
  ...
  deep-learning installation; 207 tests accompany the code.
\end{enumerate}
...
\paragraph{Data.}
A point-in-time liquidity-screened universe of $N$ US equities over
2016--2025, with delisting returns included. Headlines come from a public,
citable corpus with first-print timestamps \citep{dong2024fnspid}, so that
reviewers can rerun the text modality. We report coverage per ticker-day, the
distribution of headline counts, and class balance by year
(Table~\ref{tab:T1_main}). We do not scrape: a retrospective scrape returns
what is currently indexed rather than what was visible, and link rot,
last-modified timestamps and post-hoc editorial revision are contamination that
a publication lag does not address.
...
\ifresultsready\else
\pending{The walk-forward sweep of Section~\ref{sec:protocol} has not been
executed. The tables below are placeholders. They are populated automatically
from \texttt{paper/tables/} once \texttt{finsent/eval/report.py} has run, and
each carries a provenance footnote recording the git commit, configuration
hash, data hash and seed list that produced it.}
\fi

\resulttable{T1_main}{Dataset composition: universe size by year, ticker-days,
news coverage, headline-count distribution, and class balance by year.}
{Populated by \texttt{report.py} from the point-in-time panel.}

\resulttable{T3_main}{Predictive comparison across baselines and the proposed
model: accuracy, balanced accuracy, majority-class baseline, binary accuracy,
macro-$F_1$, Matthews correlation, rank IC with Newey--West $t$-statistics, and
Diebold--Mariano $p$-values against each baseline. Mean $\pm$ standard
...
log-likelihood; and conformal empirical coverage and mean set size at
$\alpha\in\{0.05,0.10,0.20\}$.}
{Fitted on inner-validation, evaluated on test, per fold.}
...
maximum drawdown, turnover, trade count, and paired block-bootstrap $p$-values
...
predicts that raw-softmax Kelly may fall \emph{below} equal weighting.}

\resulttable{T6_main}{Economic evaluation: gross and net Sharpe at seven cost
levels, break-even cost, annualised turnover, capacity estimate, Probabilistic
and Deflated Sharpe against the declared trial budget, Probability of Backtest
Overfitting, and factor-regression alpha with HAC $t$-statistics.}
{We do not claim a novel risk premium unless the alpha $t$-statistic clears the
threshold of \citet{harvey2016and}; if it does not, we say so.}

\resulttable{T7_main}{Ablations under the full walk-forward protocol with
confidence intervals, including the headline-shuffle null test and the
conditional (high news-flow) text ablation.}
{Reported whichever way they fall. Null results are results.}

\resulttable{T8_main}{Robustness: subperiods, sectors, horizons
$h\in\{1,5,21\}$, combinatorially purged cross-validation, and seed variance.}
{}

\resulttable{T9_main}{Gate diagnostics: regression of $\bar g_t$ on news
volume, realised volatility and earnings proximity, with significance tests.}
{This is the evidence for the mechanism described in
Section~\ref{sec:model}; without it the gate story is an unverified narrative.}
...
We commit in advance to reporting the headline-shuffle test, the generative
augmentation ablation and the candlestick ablation whichever way they fall. A
table in which every component helps is the signature of a design tuned on its
own evaluation, and referees are right to read it that way.
...
\item \textbf{Language-model pretraining leakage.} FinBERT was pretrained on
  ...
  largely unsolved problem for language-model-based financial backtests. We
  acknowledge it, and bound it where possible by reporting a subperiod strictly
  after the pretraining cutoff.
...
evaluation protocol whose controls we verified rather than asserted --- an
exercise that surfaced two failure modes, one of which inflated a spurious
information coefficient by a factor of twenty-five while remaining invisible to
every splitter.

\ifresultsready
Section~\ref{sec:results} reports what the protocol found.
\else
The empirical study specified in Section~\ref{sec:protocol} remains to be run.
We have chosen to state the protocol, the pre-declared audit thresholds and the
validation of the machinery in advance of the results rather than after them,
because a protocol published before its outcome is a commitment and one
published after it is a description.
\fi
