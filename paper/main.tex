...
\kwYearStart--\kwYearEnd{}, comprising \kwTickerDays{} ticker-days across
\kwSessions{} sessions. The screen runs at each date on information available at
that date, taking names priced above \$5 with trailing dollar volume above \$5M and
ranking them on trailing data, and we verify that it never consults a future bar.

That verification buys less than it appears to. The screen removes survivorship
bias one level down, not at the level that matters most. The pool it ranks is a set
of companies for which price history exists in the present, and it carries no
delisting returns; we had no CRSP-style database that does. A point-in-time screen
over a survivor-tilted pool inherits the tilt, and we would rather say so in the
protocol section than bury it in a footnote.

The design called for headlines from a public, citable corpus with first-print
timestamps \citep{dong2024fnspid}, so that a reviewer could rerun the text
modality. What we actually hold does not meet that bar. Of \num{5391} news rows,
\num{72} are non-empty, and those \num{72} carry \num{64} template titles across
\num{9} distinct timestamps. We therefore report the text modality as not
evaluated and run the model with the branch forced to its null embedding. Nine
timestamps cannot support a point-in-time claim, and we would be inventing one if
we tried. Scraping is not a substitute either: a retrospective scrape returns what
is indexed today, not what was visible then, and link rot, last-modified stamps and
post-hoc editorial revision are contamination that no publication lag addresses.
Table~\ref{tab:T1_main} reports the panel and states the corpus audit in its
footnote.
...
\paragraph{Baselines.}
Six models are run under the identical protocol: an explicit null signal;
buy-and-hold; time-series momentum; an $\ell_2$-regularised multinomial
logistic regression on five features; gradient-boosted trees on the same twelve
features; and \textsc{FinSentNet-C} itself. The logistic and gradient-boosted
rows are the ones that matter. If eighteen parameters or a tuned tree ensemble
ties \num{448{,}453}, that is the paper's finding and we report it as such. The
null-signal and buy-and-hold rows are controls on the plumbing rather than
competitors: a null signal that earned a Sharpe would indict the backtester,
and buy-and-hold emits a constant cross-sectional score, so its rank IC is
undefined rather than zero.

The design also called for Loughran--McDonald lexicon sentiment, an LLM-scored
sentiment scalar, and FinBERT mean-pool with the same head. All three are text
baselines and none can be run, for the reason given above: there is no usable
corpus to score. We list them here rather than deleting them, because a
baseline that disappears between the protocol and the results is
indistinguishable from one that was dropped for performing too well.

DLinear \citep{zeng2023transformers} and PatchTST \citep{nie2023time} are not
run either, and that omission is ours rather than the data's. Both are
forecasting architectures for continuous targets and adapting them to a
three-class dead-band label is a modelling decision with enough freedom in it
to constitute a separate paper; we would rather say so than report a weak
adaptation of someone else's model as though it were their result. A
replication that wants them will find the walk-forward driver takes any
callable with the same signature.

\paragraph{What we report, and what we do not.}
Reported: the class histogram and its year-by-year balance; the majority-class
baseline, balanced accuracy, macro-$F_1$, binary accuracy excluding
\Neutral{}, and Matthews correlation alongside raw accuracy; the daily
cross-sectional rank information coefficient with Newey--West $t$-statistics;
...
singleton rate against nominal level; turnover; the net-of-cost curve with the
break-even level marked; Probabilistic and Deflated Sharpe against the declared
trial budget; and the Probability of Backtest Overfitting.

Not reported, and why. A factor regression of strategy returns on market, size,
value, profitability, investment, momentum and short-term reversal requires a
point-in-time factor return file, and none accompanies this corpus; we do not
substitute a proxy series and then describe the residual as alpha. A capacity
estimate requires an impact model calibrated to executions we cannot observe,
so Section~\ref{sec:costs} reports the cost curve that a reader can recalibrate
instead of a capacity number that would look authoritative and be a guess.
Confusion matrices and decile-spread monotonicity are computed by the released
code but not typeset: for a predictor whose rank IC is indistinguishable from
zero, a confusion matrix restates the class balance and a decile spread is a
plot of noise ordered by noise. If a replication finds an edge, both become
worth printing, and both are one function call away.
...
\subsection{The online variant is what survives a regime break}
\label{sec:adaptivecoverage}

Split conformal buys its guarantee with an assumption, and the assumption is
exchangeability between the calibration block and the test block. Financial data
breaks it on a schedule. It is worth measuring what the break actually costs,
because the guarantee does not degrade gracefully when its premise fails.

We construct the failure directly. A calibration block is drawn in which the model
carries genuine information, and a test block in which the same model is wrong:
probability mass sits on a class other than the realised one, which is what a
regime change looks like to a predictor fitted before it. Under exchangeability
both variants land on nominal, within \num{0.02} at every $\alpha$ we tried. Under
the break they part company. At a nominal \num{0.90}, split coverage falls to
\num{0.467}. The Gibbs--Candès update holds \num{0.897}, and it does so without
being told that anything happened: it reads its own realised coverage and tightens
$\alpha_t$ until the shortfall closes.

Two points follow. The first is that a distribution-free guarantee is only as
distribution-free as its exchangeability assumption, and a paper that reports
split coverage on financial data without testing that assumption is reporting a
number whose premise it has not checked. The second is about ordering. An online
predictor consumes each outcome only after emitting that row's set, and if the
label reached the set first, coverage would be perfect and meaningless. Our test
suite asserts that the emitted set is unchanged across all three possible labels,
which is the only form of that check that cannot pass by accident.
...
Section~\ref{sec:lagdefect}. A lag of two would not leak but would silently
