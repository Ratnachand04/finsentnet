...
replicate, and the causes are well catalogued. Survivorship in the universe.
Overlapping labels. Unpurged splits. Unstated execution conventions. Selection
over a large and undeclared trial budget. We adopt the corresponding controls
throughout, and because a control is worth only as much as its verification, we
also report in Section~\ref{sec:validation} what happens when two of them are
switched off. Instrumentation found both failure modes; reading the code found
neither. Each would have produced a publishable-looking number, and no train/test
splitter would have objected to either.
...
modulate. Table~\ref{tab:T9_main} reports that outcome rather than omitting the
row, because a pre-specified diagnostic which disappears between the protocol
and the results is indistinguishable from one that was dropped for saying the
wrong thing.
...
volatility estimate, instead of fixing it, is what stops the label degenerating
into a volatility detector. It does not make the three classes equal. The \Up{}
share exceeds the \Down{} share in most years, because the sample covers a decade
in which US equities rose. That asymmetry is why the majority-class row in
Table~\ref{tab:T3_main} earns its place, and it is also what broke our first
attempt at position sizing. We record that failure in
Section~\ref{sec:discussion} instead of quietly repairing it, since the way it
broke is more useful than the repair.

The second is the effective sample size. Overlapping five-day labels mean the row
count overstates the information content by a factor of \kwESSFactor{}, close to
the $h+1$ the horizon implies. Every weighted loss and every standard error in
this paper uses the corrected figure. A reader comparing our sample size against a
paper that quotes raw rows should divide by that factor first.
...
instrument in the paper. One row makes the case better than any argument we
could write: the proposed model's three-class accuracy is \kwPrimaryAcc{}
against a majority-class baseline of \kwPrimaryMajority{}. It classifies
\emph{worse} than a rule that always says \Neutral{}, while simultaneously
posting the highest rank information coefficient in the table. A reader who
ranked these six models by accuracy would order them almost backwards.

That is not a paradox and it is close to the thesis of this paper. Accuracy
scores a decision the model is not asked to make. A cross-sectional book never
classifies anything; it ranks, and it trades the top against the bottom. Rank
IC with a Newey--West $t$ is the column that speaks to that, which is why the
majority-class row is mandatory rather than decorative --- without it, an
accuracy of \kwPrimaryAcc{} looks like weak evidence of something instead of
clear evidence of nothing. The buy-and-hold row emits a constant
cross-sectional score and so has no rank IC at all, and it is included as a
control on the plumbing rather than as a competitor.
...
Section~\ref{sec:discussion} anticipates. This is neither a surprise nor a
...
being perfectly uninformative simply by stating the base rate every time. That
is close to what happens here, and it is why the coverage guarantee in
...
\kwCovATenSet{} of three possible classes.

Table~\ref{tab:T4b_main} also settles a claim Section~\ref{sec:calibration} makes
and Section~\ref{sec:adaptivecoverage} demonstrates on planted data, and it
settles it on the market. Split conformal assumes exchangeability between the
calibration block and the test block, and a walk-forward over a decade of
equities does not supply it. Averaged across the $\alpha$ grid, split coverage
misses nominal by \kwCovMeanAbsSplitGap{}; the online update of
\citet{gibbs2021adaptive} misses by \kwCovMeanAbsAdaptiveGap{}, roughly four
times closer, and at the primary level it lands on \kwCovATenAdaptive{} against
a nominal \num{0.90}. We had expected to report this as a caveat and instead
report it as a result: on real data the online variant is not a refinement of
the split one, it is the one that works.
...
\IfFileExists{figures/F5_main.pdf}
  {\includegraphics[width=0.72\linewidth]{figures/F5_main.pdf}}
...
The break-even cost is the first. It answers the only question a practitioner asks
of a backtest, which is how much friction the signal absorbs before it stops
paying, and we report it in place of a single net Sharpe quoted at whichever
assumption flatters the result. A negative break-even cost is not a thin edge. It
means the book loses money gross, before a basis point is charged, and no
improvement in execution can rescue it.

The second is turnover. A decile long--short book rebalanced every five
sessions substantially replaces itself at each rebalance, and the annualised
figures in Table~\ref{tab:T6_main} reflect that. This is the arithmetic set out
in Section~\ref{sec:costs}, arriving as measurement rather than as estimate: at
this turnover a signal needs a gross edge of roughly ten basis points a day
simply to cover a ten basis point round trip, and nothing we measure comes
close.

The third is the Deflated Sharpe Ratio, computed against the declared trial
budget rather than against one. Declaring the budget is what separates a
selection procedure from data mining, and it cuts the way one would expect: we
evaluated a model zoo, two horizons beyond the primary one, and a rebalance
sweep, and the deflation charges us for all of them. We do not claim a risk
premium, novel or otherwise. The alpha $t$-statistic does not approach the
threshold of \citet{harvey2016and}, and Table~\ref{tab:T6_main} says so in the
row where a more optimistic paper would have stopped reporting.
...
\IfFileExists{figures/F7_main.pdf}
  {\includegraphics[width=0.72\linewidth]{figures/F7_main.pdf}}
  {\fbox{\parbox{0.72\linewidth}{\centering\vspace{16mm}
...
Table~\ref{tab:T7_main} reports which ablations were run and which were not,
which is a less satisfying table than the one this section was designed to
contain. We think it is the honest one.

Two of the pre-specified ablations are unavailable rather than negative. The
headline-shuffle null test and the high-news-flow conditional ablation both
require a corpus, and ours does not exist in usable form; we say so rather than
substituting a proxy that would be easier to write and impossible for a referee
to check. Their absence weakens the paper, and Section~\ref{sec:limitations}
says how much.

The architecture ablations are withheld for a stronger reason. Ablating the
fusion block, the calibration term or the variance head is only informative if
the unablated model has something to ablate; when its rank IC is
\kwPrimaryIC{} at $t=\kwPrimaryICt$, the gap between two ablations is the
difference between two noise terms. Table~\ref{tab:T7_main} makes that
condition explicit and reports the ablation as meaningful only when the base
$t$-statistic clears two, so a future run on a panel where the model does have
an edge will produce the table without anyone having to remember why it was
missing.

What survives the null is the pair of decision-layer ablations, because they do
not depend on the predictor being any good: raw against calibrated
probabilities, and gate on against gate off. Those are the rows in
Tables~\ref{tab:T5_main} and~\ref{tab:T5b_main}, and they are the ablations the
...
for a result that exists only in one configuration: the subperiod, the horizon,
and the seed. All three are swept as declared trials and counted against the
Deflated Sharpe budget in Table~\ref{tab:T6_main}, which matters more than the
sweep itself --- a robustness table assembled after the fact and not charged to
the trial count is a menu, not a control.

Seed variance is worth singling out. Across \kwSeeds{} seeds the spread in rank
IC is of the same order as the point estimates themselves, which is the
quantitative form of the warning in Section~\ref{sec:discussion}: at this sample
size and this signal level, a difference between two models that is not larger
than the difference between two seeds of one model is not a finding. It is the
reason the paired sizing experiment, and not the model comparison, is the
instrument we lean on.

Table~\ref{tab:T9_main} is the gate diagnostic, and it is empty. The fusion gate
$\bar g_t$ is a cross-modal quantity; with the news branch forced to its null
embedding there is nothing for it to modulate, so the regression of $\bar g_t$
on news volume, realised volatility and earnings proximity has no content. We
report the table empty rather than deleting it, because a pre-specified
diagnostic that quietly disappears between the protocol and the results is
exactly the pattern the protocol was written to prevent.
...
out in Table~\ref{tab:T7_main}. The unablated model's rank information
